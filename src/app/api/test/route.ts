...
export async function POST(req: Request) {
  ...
  const { text, images, gpt_thread_id } = await req.json();
  ...
  //   thread
  // );
  const result = await codeGenerator(text, gpt_thread_id, images);
  return result.toJsonResponse();
}

// async function llmChain(userInput: string, thread: Thread[]) {
//   // thread에는 모든 메시지 저장
//   // 이중에서 각 llm이 보는 메시지를 따로 분리
//   // codeGenerator는 code와 user만 필요. 근데 user도 누구에게 보냈는지에 따라 나뉨
